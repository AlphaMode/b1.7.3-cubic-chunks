...
public class BlockPatternBuilder {
    ...
    public BlockPatternBuilder aisle(String... strings) {
        if (!isEmpty((Object[])strings) && !isEmpty(strings[0])) {
    ...
    private Predicate<BlockInWorld>[][][] createPattern() {
        ...
        Predicate<BlockInWorld>[][][] predicates = (Predicate[][][])Array.newInstance(Predicate.class, new int[]{this.pattern.size(), this.height, this.width});

        for(int i = 0; i < this.pattern.size(); ++i) {
            for(int j = 0; j < this.height; ++j) {
                for(int k = 0; k < this.width; ++k) {
                    predicates[i][j][k] = (Predicate)this.lookup.get(((String[])this.pattern.get(i))[j].charAt(k));
                }
            }
        }
