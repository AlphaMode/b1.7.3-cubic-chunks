...
public class ServerHeightMap {
    ...
    /**
     * Array containing the y-coordinates of the lowest segment in each block column. The value {@link ServerHeightMap#NO_HEIGHT}
     * is used if a given block column does not contain any segments.
     */
    @Nonnull private final int[] ymin;

    /**
     * Array containing the y-coordinate of the highest segment in each block column. The value {@link ServerHeightMap#NO_HEIGHT}
     * is used if a given block column does not contain any segments.
     */
    ...
    public ServerHeightMap(byte[] heightmap) {
        this.ymin = new int[CubicChunk.SIZE * CubicChunk.SIZE];
        this.ymax = new HeightMap(heightmap);

        this.segments = new int[CubicChunk.SIZE * CubicChunk.SIZE][];

        // init to empty
        for (int i = 0; i < CubicChunk.SIZE * CubicChunk.SIZE; i++) {
            this.ymin[i] = NO_HEIGHT;
            this.ymax.set(i, NO_HEIGHT);
        }

        this.heightMapLowest = NO_HEIGHT;
    }
    ...
    public void onOpacityChange(int localX, int blockY, int localZ, int opacity) {
        if (blockY > CubicChunk.MAX_SUPPORTED_BLOCK_Y || blockY < CubicChunk.MIN_SUPPORTED_BLOCK_Y) {
            return;
        }
    ...
    public int getLowestTopBlockY() {
        if (this.heightMapLowest == NO_HEIGHT) {
    ...
    private void setNoSegmentsOpaque(int xzIndex, int blockY) {
        ...
        if (this.ymin[xzIndex] == NO_HEIGHT && this.ymax.get(xzIndex) == NO_HEIGHT) {
    ...
    private void setNoSegmentsTransparent(int xzIndex, int blockY) {
        ...
        assert !(this.ymin[xzIndex] == NO_HEIGHT || this.ymax.get(xzIndex) == NO_HEIGHT) :
        ...
        if (this.ymax.get(xzIndex) == this.ymin[xzIndex]) {
            ...
            if (blockY == this.ymin[xzIndex]) {
                this.ymin[xzIndex] = NO_HEIGHT;
                this.ymax.set(xzIndex, NO_HEIGHT);
            }
